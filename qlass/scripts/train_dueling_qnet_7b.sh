...
# Train candidate-normalized parametric-action Dueling QNet for QLASS.

model_name=Llama-2-7b-chat-hf
task=alfworld
exp_name=qlass

# Number of GPUs used by this run. Keep this in sync with CUDA_VISIBLE_DEVICES and --nproc_per_node below.
node_num=${NODE_NUM:-2}

model_path=${MODEL_PATH}  # path to the original/base models
save_dir=${MODEL_PATH}    # checkpoint save path

batch_size=${BATCH_SIZE:-64}  # если 33108 примеров в трейне, то на 2 эпохах будет 33108 * 2 / 64 = 1034 шагов.
