...
q_data_path=data/train/${task}/explore_v2/vanilla.jsonl
q_model_name=${exp_name}-${model_name}-${task}-Q_my_v2
sft_model_name=${exp_name}-${model_name}-${task}-sft_run1

export CUDA_VISIBLE_DEVICES=2,3

CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.run --nproc_per_node=${node_num} --master_port=20001 qlass/train_q.py \
