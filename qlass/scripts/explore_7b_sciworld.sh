...
CUDA_VISIBLE_DEVICES=4 python3 -m sglang.launch_server --model-path ${save_dir}${sft_model_name} --port ${sg_worker_port} >> logs/${exp_name}-sg_worker_explore0.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=5 python3 -m sglang.launch_server --model-path ${save_dir}${sft_model_name} --port ${sg_worker_port3} >> logs/${exp_name}-sg_worker_explore3.log 2>&1 &
...
for ((i=0;i<${num_workers};i=i+1)); do
    ...
    python qlass/explore_sft_agent.py \
        ...
        --num_icl_examples 0 \
        --samples_per_depth 2 \
        --output_dir data/train/${task}/explore_7b_sft_d6_i0_s2_mpr3_v2/ &
done
