...
task=sciworld # webshop, sciworld, alfworld
exp_name=qlass
...
CUDA_VISIBLE_DEVICES=2 python3 -m sglang.launch_server --model-path ${save_dir}${sft_model_name} --host "${SERVER_HOST}" --port ${sg_worker_port} --tp-size 1 >> logs/${exp_name}-sg_worker.log 2>&1 &
...
python3 -m qlass.inference --agent_config sglang_sft --model_name ${sft_model_name} --exp_config ${task} --split dev --num_icl_examples 1 --exp_name qlass_eval_sft_smart_cntxt --force_first --override --part_num 1 --part_idx 0 \
    --log_prompt_lengths \
